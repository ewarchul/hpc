...
__global__ void sum(int* result) {
    ...
    for (int i = start; i <= end; i++) {
		...
		/*
			Bez `__synchthreads()` nastapi zbyt wczesne przypisanie `partials[0]` do
			zmiennej `result` -- trzeba poczekac az zsumowane zostana wszystkie pary
			`(threadIdx.x, threadIdx.x + i)`.
		*/
